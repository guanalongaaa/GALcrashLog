//
//  GALUncaughtExceptionHandler.m
//  SmartDot
//
//  Created by guanal on 2018/6/28.
//

#import "GALUncaughtExceptionHandler.h"
...
@interface GALUncaughtExceptionHandler()
...
@implementation GALUncaughtExceptionHandler

+ (instancetype)shareInstance{
    static GALUncaughtExceptionHandler *_manager = nil;
...
- (void)uiConfig{
    ...
    NSString *filePath = [docPath stringByAppendingPathComponent:@"GALExceptionLog.txt"];
...
- (GALUncaughtExceptionHandler *(^)(BOOL isShow))showAlert{
    return ^(BOOL isShow) {
        self.isShowAlert = isShow;
        return [GALUncaughtExceptionHandler shareInstance];
    };
}

- (GALUncaughtExceptionHandler *(^)(BOOL isShow))showErrorInfor{
    return ^(BOOL isShow) {
...
- (GALUncaughtExceptionHandler *(^)(void(^ logPathBlock)(NSString *pathStr)))getlogPathBlock{
...
void HandleException(NSException *exception){
    ...
    [[GALUncaughtExceptionHandler shareInstance] performSelectorOnMainThread:@selector(handleException:) withObject:[NSException exceptionWithName:[exception name] reason:[exception reason] userInfo:userInfo] waitUntilDone:YES];
}
